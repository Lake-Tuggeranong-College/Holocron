---
isCurrent: true
needsUpdating: false
---
# Overview

The focus of this semester is to learn and develop the architecture and infrastructure required for the remainder of the projects in subsequent projects.

This includes learning about Virtualisation and Docker Containers, and how they benefit application development.

You will also be producing a report on the configuration and implementation of the server environment. Included in this report will be a timesheet detailing the process to your client as an invoice for your work.

# Schedule

Classes will run on Tuesday's each week during TRaC-X and lunchtime as workshops, or 'drop-in' class.
Classes on Thursday's (2-4pm) will cover the topics below. Assessments are due on the Thursday of the week indicated.

| Week                                     | Content                                                                                                                                                                            | Resources                                                                                    | Assessments Due   |
| ---------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------- | ----------------- |
| ![[S1 Calendar#School Events - Week 1]]  |                                                                                                                                                                                    |                                                                                              |                   |
| ![[S1 Calendar#School Events - Week 2]]  | [[Permission Notes\|Permission Notes!!!]]<br>[[Slides - Course Introduction]]<br>[[Account Creation & Software Installation]]<br>[[Software Installation & Configuration]]<br><br> | [[Slides - Development Process]]<br>![Development Process](https://youtu.be/I6SBXjgeC24)     |                   |
| ![[S1 Calendar#School Events - Week 3]]  | N/A                                                                                                                                                                                |                                                                                              |                   |
| ![[S1 Calendar#School Events - Week 4]]  | [[Slides - Virtualisation]]<br>[[`devcontainer.json` Tutorial]]<br>[[Docker Compose Tutorial]]<br>                                                                                 |                                                                                              |                   |
| ![[S1 Calendar#School Events - Week 5]]  | [[Docker Configuration]]<br>[[phpMyAdmin]]                                                                                                                                         |                                                                                              |                   |
| ![[S1 Calendar#School Events - Week 6]]  | [[Docker Configuration]]<br>[[Slides - Hardware Components]]<br>Hardware Quiz (Google Classroom)                                                                                   | ![Slides - Hardware Components](https://youtu.be/13eLcKw9DiY)                                |                   |
| ![[S1 Calendar#School Events - Week 7]]  | Assessment Submission<br><br>                                                                                                                                                      |                                                                                              | Task 1.1 Due      |
| ![[S1 Calendar#School Events - Week 8]]  | [[Slides - Markdown]]<br>[[Markdown Tutorial\|Markdown Tutorial]]<br>[[Slides - Mermaid Diagrams]]<br>[[Mermaid Tutorial\|Mermaid Tutorial]]                                       | ![Markdown & Demo](https://youtu.be/mVEOlJB1wmI)<br>![Mermaid](https://youtu.be/9p8JuvOhnAw) | Task 1.1 Resubmit |
| ![[S1 Calendar#School Events - Week 9]]  | [[Software Installation & Configuration Manual]]                                                                                                                                   |                                                                                              |                   |
| ![[S1 Calendar#School Events - Week 10]] | [[Software Installation & Configuration Manual]]                                                                                                                                   |                                                                                              |                   |
| ![[S1 Calendar#School Events - Week 11]] | [[Software Installation & Configuration Manual]]                                                                                                                                   |                                                                                              |                   |
| ![[S1 Calendar#School Events - Week 12]] | [[Software Installation & Configuration Manual]]                                                                                                                                   |                                                                                              |                   |
| ![[S1 Calendar#School Events - Week 13]] | [[Software Installation & Configuration Manual]]                                                                                                                                   |                                                                                              | Task 1.2 Due      |
| ![[S1 Calendar#School Events - Week 14]] | [[Spreadsheets]]                                                                                                                                                                   | ![Task 1.3 Spreadsheets](https://youtu.be/RdUzSmqdyi4)                                       |                   |
| ![[S1 Calendar#School Events - Week 15]] | Spreadsheet formula                                                                                                                                                                |                                                                                              |                   |
| ![[S1 Calendar#School Events - Week 16]] | Timesheet                                                                                                                                                                          |                                                                                              | Task 1.3 Due      |
| ![[S1 Calendar#School Events - Week 17]] | Assessment Second Sitting<br>VET Learner Questionnaire                                                                                                                             |                                                                                              |                   |
| ![[S1 Calendar#School Events - Week 18]] | Assessment Second Sitting                                                                                                                                                          |                                                                                              |                   |
| ![[S1 Calendar#School Events - Week 19]] |                                                                                                                                                                                    |                                                                                              |                   |
| ![[S1 Calendar#School Events - Week 20]] |                                                                                                                                                                                    |                                                                                              |                   |




# Tasks

## Task 1.1 - Docker Installation and Configuration
### Task Overview
This assignment requires you to demonstrate your understanding of containerisation by deploying a multi-service web application using Docker and Docker Compose. You will provision a Docker environment containing a website and a database, configure them to work together,

Using `docker-compose`, create a multi-service environment, according to the task specifications.

Your `docker-compose.yml` file must:
- Define the services.
- Properly configure network connectivity between the website and the database containers.
- Use volumes to ensure data persistence for the database.
- Set necessary environment variables (e.g., database credentials).

### Deliverable/s

| Deliverables                   | Submitted via  |
| ------------------------------ | -------------- |
| Docker Container configuration | GitHub project |

### Task 1.1 Competencies

| Unit Code                                                                        | Unit Title                                        | Cert II Core | Cert II Elective | Cert III Core | Cert III Elective |
| -------------------------------------------------------------------------------- | ------------------------------------------------- | :----------: | :--------------: | :-----------: | :---------------: |
| [ICTICT213](https://training.gov.au/Training/Details/ICTICT213/unitdetails)	<br> | Use computer operating systems and hardware       |      ✅       |                  |               |         ✅         |
| [ICTICT223](https://training.gov.au/Training/Details/ICTICT223/unitdetails)      | Install software applications                     |              |        ✅         |               |                   |
| [ICTICT214](https://training.gov.au/Training/Details/ICTICT214/unitdetails)      | Operate application software packages (Element 4) |      ✅       |                  |               |         ✅         |

### Assessment Task

https://classroom.google.com/c/NzkyMDMwMjMzMDEy/a/ODQ1NDI4MDg0MDI0/details

Hardware Quiz : https://docs.google.com/forms/d/e/1FAIpQLSdROuBbDqSfSs5Vv_TXLdyMnGG2UfAjavmXHxyZ9-DEB9ar1w/viewform?usp=dialog&authuser=0


---
## Task 1.2 - Report
### Task Overview

You are required to write a technical report in a single Markdown file that documents the installation and configuration of a Docker environment. The report must provide a detailed, step-by-step guide, including the complete `docker-compose.yml`file with explanations for each part, commands to manage the containers, and instructions for verifying the setup. Additionally, the report should list all necessary software and tools, and it must contain at least two Mermaid diagrams to visually represent the container architecture and service communication.

### Deliverable/s

| Deliverables       | Submitted via |
| ------------------ | ------------- |
| Technical Document | GitHub        |

### Task 1.2 Competencies

| Unit Code                                                                   | Unit Title                                             | Cert II Core | Cert II Elective | Cert III Core | Cert III Elective |
| --------------------------------------------------------------------------- | ------------------------------------------------------ | :----------: | :--------------: | :-----------: | :---------------: |
| [ICTICT214](https://training.gov.au/Training/Details/ICTICT214/unitdetails) | Operate application software packages (Elements 1 & 2) |      ✅       |                  |               |         ✅         |
| [BSBTEC201](https://training.gov.au/training/details/BSBTEC201/unitdetails) | Use business software applications                     |              |        ✅         |               |         ✅         |


### Task 1.2 Assessment Task

https://classroom.google.com/c/NzkyMDMwMjMzMDEy/a/ODU2NDE5NjMwMjc3/details


## Task 1.3 - Invoice
### Task Overview

You are to submit a invoice, using a provided Google Sheet (on Google Classroom). The invoice should 
reflect the amount of hours you have worked on Project 1, and be used to submit to your client to be paid.

You will build the invoice, including adding specific details and then submit as part of the GitHub Repository.
### Deliverable/s

| Deliverables                        | Submitted via |
| ----------------------------------- | ------------- |
| Spreadsheet as a XLSX **and** a PDF | GitHub        |

### Task 1.3 Competencies

| Unit Code                                                                   | Unit Title                                        | Cert II Core | Cert II Elective | Cert III Core | Cert III Elective |
| --------------------------------------------------------------------------- | ------------------------------------------------- | :----------: | :--------------: | :-----------: | :---------------: |
| [ICTICT214](https://training.gov.au/Training/Details/ICTICT214/unitdetails) | Operate application software packages (Element 3) |      ✅       |                  |               |         ✅         |

### Task 1.3 Assessment Task

https://classroom.google.com/c/NzkyMDMwMjMzMDEy/a/ODA5Nzc1MDkyOTUy/details
