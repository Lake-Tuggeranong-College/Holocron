This Rapid Application Development (RAD) phase of the project involves quickly implementing code which transmits telemtry data to and from the server from the Adafruit Huzzah32 Feather (ESP32) device.

1. Open the project. The focus for this phase is the Arduino code.
2. Create a new file `src/comms.h`. Replace the contents with:
```arduino
/*
...
```
4. Create a new file `src/sensitiveInformation.h`. Replace the contents with this:
```arduino
/*
...
6. Update `main.cpp` with the following code:
```arduino
// MQTT client name
...
```
7. Change the `mqttClient` variable to something unique. For example: `const char *mqttClient = "ESP32-Ryan";`
8. Select **File->Save All**.
9. Select the Build button.
![[moduleRADBuild.png]]
